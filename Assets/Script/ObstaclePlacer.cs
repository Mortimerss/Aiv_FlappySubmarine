...
public class ObstaclePlacer : MonoBehaviour
{
    ...
    private bool _canPlaceObstacles = true;
    ...
    void PlaceObstacles()
    {
        if (!_canPlaceObstacles) return; 

        GameObject prefab = obstaclePrefabs[Random.Range(0, obstaclePrefabs.Length)];
        GameObject obstacle = Instantiate(prefab,
            _currentPosition + distance * movementDirection + displacement + new Vector3(0, Random.Range(0, randomVerticalDisplacement), 0),
    ...
    public void ResetObstaclePlacer()
    {
        _currentPosition = _startPosition;
        _canPlaceObstacles = true;
    }
    public void StopPlacingObstacles()
    {
        _canPlaceObstacles = false;
    }
}